...
public class JuggernautSpawner extends BukkitRunnable {
    ...
    public JuggernautSpawner(Main plugin) {
        this.plugin = plugin;
    }
    ...
    public void run() {
        ...
        for (Player p : players) {
            ...
            int nearbyJuggernaut = 0;
            for (Entity e : world.getEntities()) {
                if (e instanceof Zombie && e.getCustomName() != null && e.getCustomName().equals("Juggernaut")) {
                    Location JuggernautLocation = e.getLocation();
                    double distance = playerLocation.distance(JuggernautLocation);
                    if (distance < GRID) {
                        nearbyJuggernaut++;
                    }
                }
